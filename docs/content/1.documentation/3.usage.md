...
2. **Use with playbook**:
   ```bash
   ansible-playbook ./playbook.yaml -K -e github_token=YOUR_TOKEN_HERE
   ```
...
```bash
ansible-playbook ./playbook.yaml -K -e "git_user_email='your@email.com'" -e "git_user_name='Your Name'"
```
...
```bash
ansible-playbook ./playbook.yaml -K -e=gui=true -e=dev_tools_gui=true
```
...
```bash
ansible-playbook ./playbook_macos.yaml -K -e "git_user_email='your@email.com'" -e "git_user_name='Your Name'"
```

#### Install GUI tools

```bash
ansible-playbook ./playbook_macos.yaml -K -e=gui=true -e=dev_tools_gui=true
```
...
- `-e username="your_user"`: (Optional) Sets the username. Defaults to the current user invoking the playbook.
